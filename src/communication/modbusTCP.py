...
from src.communication.com_protocol import ComProtocol, ComProtocolError, \
    ComProtocolValueError, ComProtocolConnectionError


class ModbusTCP(ComProtocol, ABC):
    log = logging.getLogger(__name__)

    def __init__(self, settings):
        super(ModbusTCP, self).__init__(settings['module-name'])

        # SETTINGS

        # needed - config: [
        #     module-name, ip, port, framer
        # ]
        ...
        framer = ModbusTCP.get_framer(settings['framer'])
        timeout = float(settings['timeout']) if 'timeout' in settings else 0.2

        # classpymodbus.client.sync.ModbusTcpClient(host='127.0.0.1', port=502, framer=<class 'pymodbus.framer.socket_framer.ModbusSocketFramer'>, **kwargs)
        self._client = ModbusTcpClient(host, port, framer, timeout=timeout)
        self.log.info(f'{self.name} modbus connection to {host}:{port} and with {framer}')
    ...
    def get_framer(str_framer):
        ...
        elif str_framer == 'ModbusTlsFramer':
            return ModbusTlsFramer
    ...
    def read(self, address, length, register_type, unit=None):
        ...
        try:
            if register_type == 'co':
                return ModbusTCP.get_coil_response(self._client.read_coils(address, length, unit=unit))
            elif register_type == 'di':
                return ModbusTCP.get_coil_response(self._client.read_discrete_inputs(address, length, unit=unit))
            elif register_type == 'ir':
                return ModbusTCP.get_register_response(self._client.read_input_registers(address, length, unit=unit))
            elif register_type == 'hr':
                return ModbusTCP.get_register_response(self._client.read_holding_registers(address, length, unit=unit))
        except ConnectionException:
            raise ComProtocolConnectionError('impossible to reconnect modbusTCP')
        except AttributeError:
            self._client.close()
            raise ComProtocolValueError('problem with modbusTCP transaction')
        else:
    ...
    def read_value(self, comm_parameters, attribute):
        ...
        if 'unit-identifier' in comm_parameters:
            return self.read(address, length, register_type, comm_parameters['unit-identifier'])
        else:
